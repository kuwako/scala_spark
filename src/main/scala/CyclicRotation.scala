...
object CyclicRotation {
  def main(args: Array[String]): Unit = {
    println(solution(Array(1, 2, 3, 5), 5).toList)
  }
  ...
  def solution(a: Array[Int], k: Int): Array[Int] = {
    ...
    calc(a.toList, num).toArray
  }

  def calc(list: List[Int], num: Int): List[Int] = list match {
    case xs :+ x if num > 0 => calc(x :: xs, num - 1)
    case xs :+ x if num == 0 => list
